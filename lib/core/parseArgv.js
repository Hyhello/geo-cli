...
const parseArgv = (args, opts, command) => {
    ...
    // --pretty
    if (opts.pretty) {
        config.indent = config.pretty = opts.pretty;
    ...
    const filenames = input.reduce(function (globbed, dir) {
        if (!checkPathExists(dir)) {
            ...
            if (isDirectory(dir)) {
                const pattern = !opts.recursive ? `${dir}/*${SUFFIX}` : `${dir}/**/*${SUFFIX}`;
    ...
    if (output.length) {
        ...
        if (output.length === 1 && !isDir) {
            config.input.forEach((file, index) => {
                output[index] = isDir ? file : path.join(dir, opts.relative ? file : path.basename(file));
            });
